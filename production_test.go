// The purpose of production test is just to verify everything works as expected
// Think production test as a kind of sanity test.
...
func (s ProductionTestSuite) Test_Hello_ReturnsStatus200() {
	...
	if len(os.Getenv("DURATION")) > 0 {
		...
		for time.Since(start).Minutes() < max {
			address := fmt.Sprintf("http://%s/hello", s.hostIP)
